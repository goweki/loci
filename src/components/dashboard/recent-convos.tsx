...
import { getRecentConversationsAction } from "@/actions/conversation.actions";
...
export default function RecentConvos() {
  ...
  const fetchRecentConvos = useCallback(async () => {
    const resConvos = await getRecentConversationsAction();

    if (!resConvos.ok) {
      toast.error(resConvos.error);
    } else setRecentConversations(resConvos.data);
  }, []);

  useEffect(() => {
    startTransition(async () => {
      await fetchRecentConvos();
    });
  }, [fetchRecentConvos]);
